...
public class SimplePetriNetSolverPlacesGoal {
	...
	public SimplePetriNetSolverPlacesGoal(List<TransitionPetriNet> modelExt, List<PlacePetriNet> placesExt,
		...
		for (String logExt : logsExt) {
			...
			if (activityInModel(logExt)) {
				if (!logs.contains(logExt)) {
					...
					TransitionPetriNet t = new TransitionPetriNet(model.get(indexTransitionInVarModel(logAux)), logExt,
							namePrime);
					model.add(t);
	...
	public void createVariables() throws IloException {
		...
		CountNumCeros = new IlcIntVar[model.size()];

		// the worst case is: the longest path plus the log size
		// TODO
		int maxNumCeros = 59;
		int minNumCeros = 14;
		n = maxNumCeros + logs.size();// logs.size();//model.size();//logs.size()
										// + logs.size()/5;//model.size();// +
										// logs.size();
		// jump = n;//logs.size();
		int error = model.size();// n;//logs.size();

		// System.out.println("-------------N:" + n);
		VarAlignment = solver.intVar(0, maxNumCeros, " Alingment");
		sumCountCeros = solver.intVar(model.size() - maxNumCeros, model.size() - minNumCeros, " sumCountCeros");
		// sumCountCeros = solver.intVar(0, model.size(), " sumCountCeros");
		for (int i = 0; i < model.size(); i++) {
			...
			CountNumCeros[i] = solver.intVar(0, 1, model.get(i).getName() + " CountNumCeros");
		...
		for (int i = 0; i < model.size(); i++) {
			for (int j = 0; j < model.size(); j++) {
				if (i == startTransition) {
					if (mandatory[i][j] == 1) {
						solver.add(solver.gt(VarModel[j], floyd[i][j]));
					}
				} else {
					if (mandatory[i][j] == 1 && floyd[i][j] > 0 && i != j) {

						solver.add(solver.imply(solver.neq(VarModel[i], 0),
								solver.le(solver.sum(floyd[i][j], VarModel[i]), VarModel[j])));
					}
				}
	...
	public void obtainFloydMatrix() {
		...
		// for every intermediate place, put '1' in the floid matrix, and -1
		// otherwise
		 ...
		 * for (int j=0;j<model.size();j++){
		 * 
		 * System.out.print(" � "+mandatory[i][j]); } System.out.println();
		 * 
		 * }
		 */ /*
			 * System.out.
			 * println("Start - Matriz de adyacencia correspondiente: "); for
			 ...
			 */
		/*
		 * System.out.println("Start - Mandatory Matrix "); for (int
		 * i=0;i<model.size();i++){ for (int j=0;j<model.size();j++){
		 * System.out.print(" � "+mandatory[i][j]); } System.out.println(); }
		 */
	}
	...
	// index of transaction in VarModel for a transaction Id. For duplicated
	// transactions too
	...
	// index of LIST of transaction in VarModel for a transaction Id. For
	// duplicated transactions too
	...
	public void startPlace(PlacePetriNet place) throws IloException {
		...
		// solver.distribute(arg0, arg1, arg2)
	...
	public void intermediatePlace(PlacePetriNet place) throws IloException {
		...
		for (Integer i : indexsIn) {
			for (Integer j : indexsOut) {
				// solver.add(solver.imply(solver.neq(VarModel[j],
				// 0),solver.and(solver.lt(VarModel[j], solver.sum(VarModel[i],
				// jump)), solver.gt(VarModel[j], VarModel[i]))));
		...
		if (indexsIn.size() == 1 && indexsOut.size() == 1) {
			...
			solver.add(solver.ifThenElse(solver.neq(VarModel[indexsIn.get(0)], 0),
					solver.gt(VarModel[indexsOut.get(0)], VarModel[indexsIn.get(0)]),
					solver.eq(VarModel[indexsOut.get(0)], 0)));
			...
			if (indexsIn.size() == 1) {
				if (indexsOut.size() == 2) {
					solver.add(solver.ifThenElse(solver.neq(VarModel[indexsIn.get(0)], 0),
							solver.xor(solver.gt(VarModel[indexsOut.get(0)], VarModel[indexsIn.get(0)]),
									solver.gt(VarModel[indexsOut.get(1)], VarModel[indexsIn.get(0)])),
							solver.and(solver.eq(VarModel[indexsOut.get(1)], 0),
									solver.eq(VarModel[indexsOut.get(0)], 0))));
				} else {
					for (int i = 0; i < indexsOut.size(); i++) {
					...
					solver.add(solver.ifThenElse(solver.neq(VarModel[indexsIn.get(0)], 0),
							solver.eq(solver.sum(reifVarModelOut), 1), solver.eq(solver.sum(reifVarModelOut), 0)));
					solver.add(solver.ifThenElse(solver.eq(solver.sum(reifVarModelOut), 1),
							solver.neq(VarModel[indexsIn.get(0)], 0), solver.neq(VarModel[indexsIn.get(0)], 1)));
				}

			} else if (indexsOut.size() == 1) {
				if (indexsIn.size() == 2) {

					solver.add(solver.ifThenElse(solver.eq(VarModel[indexsOut.get(0)], 0),
							solver.and(solver.eq(VarModel[indexsIn.get(1)], 0),
									solver.eq(VarModel[indexsIn.get(0)], 0)),
							solver.xor(solver.eq(VarModel[indexsIn.get(0)], 0),
									solver.eq(VarModel[indexsIn.get(1)], 0))));
				} else {
					for (int i = 0; i < indexsIn.size(); i++) {
					...
					solver.add(solver.ifThenElse(solver.eq(solver.sum(reifVarModelIn), 1),
							solver.gt(VarModel[indexsOut.get(0)], 0), solver.eq(VarModel[indexsOut.get(0)], 0)));
					solver.add(solver.ifThenElse(solver.gt(VarModel[indexsOut.get(0)], 0),
							solver.eq(solver.sum(reifVarModelIn), 1), solver.eq(solver.sum(reifVarModelIn), 0)));
				}
			} else {
				for (int i = 0; i < indexsIn.size(); i++) {
					reifVarModelIn[i] = solver.intVar(0, 1, indexsIn.get(i) + "reifModelIn");
	...
	// Every activity ai appearing in the case (from the event log) to check,
	// but
	...
	public IlcSolver DifferenceAligmentAndMinimize() throws IloException {
		...
		for (int i = 0; i < model.size(); i++) {
			solver.add(solver.ifThenElse(solver.eq(VarLog[i], VarModel[i]), solver.eq(VarDifference[i], 0),
					solver.ifThenElse(solver.and(solver.neq(0, VarLog[i]), solver.neq(0, VarModel[i])),
							solver.eq(VarDifference[i], 2), solver.eq(VarDifference[i], 1))));

			// solver.add(solver.imply(solver.eq(VarLog[i], VarModel[i]),
			// solver.eq(VarDifference[i], 0)));

			// iguales y distintos de 0
			solver.add(solver.ifThenElse(solver.eq(0, VarModel[i]), solver.eq(CountNumCeros[i], 1),
					solver.eq(CountNumCeros[i], 0)));
			/*
			 * //iguales e iguales a 0
			 * solver.add(solver.imply(solver.and(solver.eq(VarLog[i],
			 * VarModel[i]), solver.eq(VarLog[i], 0)),
			 * solver.eq(CountDifferenceNum[i], 0))); //distintos y los dos
			 * distintos de 0
			 * solver.add(solver.imply(solver.and(solver.neq(VarLog[i],
			 * VarModel[i]), solver.and(solver.neq(VarLog[i], 0),
			 * solver.neq(VarModel[i], 0))), solver.eq(CountDifferenceNum[i],
			 * 2))); //distintos y s�lo 1 distinto de 0
			 * solver.add(solver.imply(solver.and(solver.neq(VarLog[i],
			 * VarModel[i]), solver.xor(solver.neq(VarLog[i], 0),
			 * solver.neq(VarModel[i], 0))), solver.eq(CountDifferenceNum[i],
			 * 1)));
			 */
		...
		solver.add(solver.eq(solver.sum(CountNumCeros), sumCountCeros));
		// solver.add(solver.le(solver.max(VarModel), solver.sum(VarAlignment,
		// 160)));
		// solver.add(solver.le(solver.max(VarLog), solver.sum(VarAlignment,
		// 160)));
	...
	public void search(FileWriter fichero) throws IloException {
		...
		/*
		 * IlcIntVar[] ordered = new IlcIntVar[model.size()];
		 * 
		 * 
		 * int max=0; for(int i=0;i<floyd.length;i++){ ordered[i] =
		 * solver.intVar(0, n, model.get(i).getName()+" Log");
		 * if(floyd[startTransition][i]>max) max = floyd[startTransition][i]; }
		 * System.out.println("max " + max); int pos =0; for(int
		 * i=0;i<=max;i++){ for(int j=0;j<floyd.length;j++){
		 * if(floyd[startTransition][j]==i){ solver.add(solver.eq(VarModel[j],
		 * ordered[pos])); pos++; } } }
		 */
		IlcGoal g = solver.generate(VarLog);

		solver.newSearch(g);

		// solver.newSearch();
		/*
		 * System.out.print("Model["); for(TransitionPetriNet t: model){
		 * System.out.print(t.getId()+","); }System.out.println("]");
		...
		while (solver.next()) {
			...
			for (int i = 0; i < VarModel.length; i++) {
				...
				// System.out.println(((IlcIntVar)VarModel[i]).getName()+ " " +
				// VarModel[i]+ " :" + VarLog[i]);
				count++;
				/*
				...
				// System.out.println(VarLog[i]+" :" + VarDifference[i] + ",");

				// System.out.println("valor del total " +
				// ((IlcIntVar)VarDifference[i]).getName() +" :" +
				// VarDifference[i]);

			}

			//
			// System.out.println("Total " + ((IlcIntVar)VarAlignment).getName()
			// +" :" + VarAlignment);
			double current_time_end = System.currentTimeMillis();
			// System.out.println("iter : " + iter + " VarAligment:" +
			// VarAlignment + " Tiempo de ejecuci�n Durante la b�squeda:
			// "+((double)current_time_end-(double)time_start)/1000+ " Count
			// Ceros:" + this.sumCountCeros);
			// System.out.println("Tiempo de ejecuci�n Durante la b�squeda:
			// "+((double)current_time_end-(double)time_start)/1000);

			for (int i = 0; i < VarModel.length; i++) {
				ultimaIteracion = ultimaIteracion + "\n" + ((IlcIntVar) VarModel[i]).getName() + " " + VarModel[i]
						+ " :" + VarLog[i];
				ultimaIteracion = ultimaIteracion + "\n" + "valor del total " + ((IlcIntVar) VarDifference[i]).getName()
						+ " :" + VarDifference[i];

			}

			ultimaIteracion = ultimaIteracion + "\n" + "Total " + ((IlcIntVar) VarAlignment).getName() + " :"
					+ VarAlignment;
		...
		ultimaIteracion = ultimaIteracion + "\n" + "Log :  " + logs.size();
		ultimaIteracion = ultimaIteracion + "\n" + "In log, not in Model activities size :  "
				+ activityInlogNotInModel.size();
		ultimaIteracion = ultimaIteracion + "\n" + "Activities duplicated in log :  " + duplicatedInLog.size();
		ultimaIteracion = ultimaIteracion + "\n" + "Tiempo de ejecuci�n :  "
				+ ((double) time_end - (double) time_start) / 1000;
